...
module PoiseLanguages
  ...
  # poise-python) that install from RedHat SCL.
  #
  # @since 1.0.0
  # @example
  #   module PoisePython
  ...
  module SclProviderMixin
    ...
    def scl_package
      @scl_package ||= self.class.find_scl_package(node, options['version'])
    end
    ...
    def install_scl_repo_package
      raise PoiseLanguages::Error.new("No SCL repoistory package for #{node['platform']} #{node['platform_version']}") unless scl_package
      rpm_package 'rhscl-' + scl_package[:name] do
        source scl_package[:url]
      end
    end

    def install_scl_package
      yum_package scl_package[:name] do
